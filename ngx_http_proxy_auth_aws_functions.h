...
ngx_str_t *ngx_http_proxy_auth_aws_compute_request_time(ngx_http_request_t *r,
    time_t *timep);
int ngx_http_proxy_auth_aws_cmp_keys(const void *one, const void *two);
ngx_int_t ngx_http_proxy_auth_aws_is_already_encoded(u_char *data, size_t len);
ngx_str_t *ngx_http_proxy_auth_aws_canonize_query_string(ngx_http_request_t *r,
    ngx_str_t *args);
ngx_http_proxy_auth_aws_canon_header_t ngx_http_proxy_auth_aws_canonize_headers(
    ngx_http_request_t *r, ngx_str_t *host, ngx_str_t *amz_date,
    ngx_str_t *content_hash);
ngx_str_t *ngx_http_proxy_auth_aws_body_hash(ngx_http_request_t *r);
ngx_int_t ngx_http_proxy_auth_aws_escape_uri(ngx_http_request_t *r,
    ngx_str_t *src);
ngx_str_t *ngx_http_proxy_auth_aws_canon_uri(ngx_http_request_t *r,
    ngx_str_t *path);
ngx_http_proxy_auth_aws_canon_req_t
...
ngx_str_t *ngx_http_proxy_auth_aws_string_to_sign(ngx_http_request_t *r,
    ngx_str_t *key_scope, ngx_str_t *date, ngx_str_t *canon_request_hash);
ngx_str_t *ngx_http_proxy_auth_aws_make_auth_token(ngx_http_request_t *r,
...
ngx_http_proxy_auth_aws_signed_req_t ngx_http_proxy_auth_aws_compute_signature(
    ngx_http_request_t *r, ngx_str_t *signing_key, ngx_str_t *key_scope,
    ngx_str_t *host, ngx_str_t *uri, ngx_str_t *method);
ngx_int_t ngx_http_proxy_auth_aws_generate_signing_key(ngx_http_request_t *r,
    ...
    ngx_str_t *key_scope);
ngx_array_t *ngx_http_proxy_auth_aws_sign(ngx_http_request_t *r,
    ngx_str_t *access_key, ngx_str_t *signing_key, ngx_str_t *key_scope,
    ngx_str_t *secret_key, ngx_str_t *region, ngx_str_t *host, ngx_str_t *uri,
    ngx_str_t *method);
